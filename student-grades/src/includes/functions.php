...
function isLoggedIn() {
    if (!isset($_SESSION['user_id'])) {
         // Maybe redirect here too, or let requireRole handle it
         // redirect('/student-grades/src/index.php'); // Uncomment if needed
         return false;
    }
    return true;
}
...
function redirect($url) {
    // Define the base path for the login page relative to the project root
    $login_page = '/student-grades/src/index.php'; // Or wherever your main login is

    // If the provided URL is simple (like 'index.php'), assume it means the login page
    if ($url === 'index.php') {
         header("Location: " . $login_page);
    } else {
        // Otherwise, use the provided URL (might be a dashboard like 'teacher.php')
         header("Location: " . $url);
    }
    exit;
}
...
function requireRole($required_role) {
    if (!isLoggedIn() || !hasRole($required_role)) {
         // Explicitly redirect to the correct login page from anywhere
         redirect('/student-grades/src/index.php'); // Use absolute path from web root
    }
}
